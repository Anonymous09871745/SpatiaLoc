...
def plot_objects(objects, pose=None, scale=1024):
    img = np.full((scale, scale, 3), 255, dtype=np.uint8)
    for obj in objects:
        c = CLASS_TO_COLOR[obj.label]
        for point in obj.xyz:
            point = np.intp((point[0:2] + 0.5) * scale / 2)
            cv2.circle(img, tuple(point), 1, (int(c[2]), int(c[1]), int(c[0])))
    if pose is not None:
        point = np.intp((pose[0:2] + 0.5) * scale / 2)
        cv2.circle(img, tuple(point), scale // 100, (255, 0, 0), thickness=5)
...
def plot_matches_in_best_cell(
    ...
    # Draw pose
    point = np.intp(pose.pose[0:2] * scale)
    ...
    for i_obj, obj in enumerate(cell_objects):
        ...
        target = np.intp(obj.get_closest_point(pose.pose) * scale)[0:2]
        cv2.arrowedLine(
            img,
            tuple(point),
            tuple(target),
...
def plot_pose_in_best_cell(cell: Cell, pose: Pose, scale=1024, use_rgb=False, show_unmatched=False):
    ...
    for descr in pose.descriptions:
        ...
        target = np.intp(descr.closest_point[0:2] * scale)
...
def plot_cells_and_poses(cells: List[Cell], poses: List[Pose], size=1024):
    ...
    for pose in poses:
        ...
        p = np.intp(p / scale * size)
        cv2.circle(img, (p[0], p[1]), 6, (0, 0, 255), thickness=4)
    img = cv2.flip(img, 0)
    return img

def plot_cells_and_poses_pred_and_gt (cells: List[Cell], poses: List[Pose], pred_poses:List[Pose], size=1024, show_pred=True):
    best_cell_ids = [pose.cell_id for pose in poses]
    pose_locations = np.array([pose.pose_w for pose in poses] + [pose.pose_w for pose in pred_poses])
    min_x, max_x, min_y, max_y = (
        np.min(pose_locations[:, 0]),
        np.max(pose_locations[:, 0]),
        np.min(pose_locations[:, 1]),
    ...
    if not show_pred:
        if len(poses) > 1:
            for i in range(len(poses) - 1):
                p1 = poses[i].pose_w - np.array((min_x, min_y, 0))
                p2 = poses[i + 1].pose_w - np.array((min_x, min_y, 0))
                p1 = np.intp(p1 / scale * size)
                p2 = np.intp(p2 / scale * size)
                cv2.circle(img, (p1[0], p1[1]), 6, (0, 0, 0), thickness=1)
    else:
        #show pred result on trajectory
        for i in range(len(poses)):
            gt_poses = poses[i]
            pose = pred_poses[i]
            dist = np.linalg.norm(gt_poses.pose_w[0:2] - pose.pose_w[0:2])
            p = pose.pose_w - np.array((min_x, min_y, 0))
            p = np.intp(p / scale * size)
            if(dist<15):
                cv2.circle(img, (p[0], p[1]), 4, (255, 0, 0), thickness=2)
            else:
                cv2.circle(img, (p[0], p[1]), 4, (0, 0, 255), thickness=2)
