...
def calc_pose_error_intersect(objects, matches0, poses: List[Pose], directions):
    ...
    poses = np.array([pose.pose for pose in poses])[
        :, 0:2]  # Assuming this is the best cell!
    ...
    def forward(self, im, s):  # Norming the input (as in paper) is actually not helpful
        ...
        cost_s = torch.max(
            torch.autograd.Variable(torch.zeros(
                scores.size()[0], scores.size()[1]).cuda()),
        ...
        # Take mean for batch-size stability
        return (cost_s.sum() + cost_im.sum()) / len(im)
...
class HardestRankingLoss(torch.nn.Module):
    ...
    def forward(self, images, captions):
        ...
        similarity_scores = torch.mm(
            images, captions.transpose(1, 0))  # [I x C]
...
class ContrastiveLoss_input_score(torch.nn.Module):
    def __init__(self, temperature=1.0):
        super(ContrastiveLoss_input_score, self).__init__()
        # self.margin = margin
        self.temperature = temperature

    def forward(self, score):  # Norming the input (as in paper) is actually not helpful
...
class LinearCritic(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        sim11 = self.cossim(
            z1.unsqueeze(-2), z1.unsqueeze(-3)) / self.temperature
        sim22 = self.cossim(
            z2.unsqueeze(-2), z2.unsqueeze(-3)) / self.temperature
        sim12 = self.cossim(
            z1.unsqueeze(-2), z2.unsqueeze(-3)) / self.temperature
        d = sim12.shape[-1]
...
class CCL(nn.Module):
    ...
    def __init__(self, temperature=0.05, method='log', q=0.5, ratio=0, alpha=2):
    ...
    def forward(self, im, s):
        ...
        n = scores.shape[0]
        num = n - \
            1 if self.ratio <= 0 or self.ratio >= 1 else int(self.ratio * n)
        V, K = randn.topk(num, dim=1)
        mask = torch.zeros_like(scores)
        mask[torch.arange(n).reshape([-1, 1]).cuda(), K] = 1.

        # calculate feature distance
        dist = abs(torch.norm(im.unsqueeze(1)-s.unsqueeze(0), dim=2))
        dist = dist/dist.max()
        dist = pow((1-dist+1e-6), 1/self.alpha)
        dist[dist > 1] = 1
        dist[dist < self.thresh] = self.thresh

        if self.method == 'log':
            def criterion(x): return -((1. - x + eps).log())  # distance
        elif self.method == 'tan':
            def criterion(x): return (x.tan())
        elif self.method == 'abs':
            def criterion(x): return (x)
        elif self.method == 'exp':
            def criterion(x): return ((-(1. - x)).exp())
        elif self.method == 'gce':
            def criterion(x): return ((1. - (1. - x + eps) ** self.q) / self.q)
        else:
            raise Exception('Unknown Loss Function!')
        # return (criterion(i2t)*mask).sum(1).mean() + (criterion(t2i)*mask).sum(1).mean()
        return (criterion(i2t) * dist*mask).sum(1).mean() + (criterion(t2i) * dist.permute(1, 0)*mask).sum(1).mean()


class CCL_input_score(nn.Module):
    """
    Compute contrastive loss
    """

    def __init__(self, temperature=0.05, method='log', q=0.5, ratio=0, alpha=2):
        super(CCL_input_score, self).__init__()
        self.tau = temperature
        self.method = method
        self.q = q
        self.ratio = ratio
    ...
    def forward(self, score, dist):
        ...
        # print(dist.mean())
        if self.method == 'log':
            def criterion(x): return -((1. - x + eps).log())
        elif self.method == 'tan':
            def criterion(x): return (x.tan())
        elif self.method == 'abs':
            def criterion(x): return (x)
