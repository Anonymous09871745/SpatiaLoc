...
from training.losses import calc_pose_error2 as calc_pose_error


def train_epoch(model, dataloader, args, model_coarse=None):
    model.train()
    ...
    for i_batch, batch in pbar:
        ...
        if model_coarse is not None:
            text_enc, text_enc_object_level, text_enc_relation_level = model_coarse.encode_text(
                texts)
            cell_enc, cell_enc_object_level, cell_enc_relation_level, cell_enc_object_level_mask, cell_enc_relation_level_mask = model_coarse.encode_objects(
            ...
            output = model(batch["objects"], texts, batch["object_points"],
                           object_level_similarity[:, :, 0:args.pad_size], args.confidence_thresh)
        ...
        loss_offsets = criterion_offsets(
            output, torch.tensor(
                np.array(batch["offsets"]), dtype=torch.float, device=device)
        )
...
@torch.no_grad()
def eval_epoch(model, dataloader, args, model_coarse=None):
    model.eval()
...
if __name__ == "__main__":
    ...
    if args.dataset == "K360":
        ...
        dataset_val = Kitti360FineDatasetMulti(
            args.base_path, SCENE_NAMES_VAL, val_transform, args,)
        dataloader_val = DataLoader(
            dataset_val, batch_size=args.batch_size, collate_fn=Kitti360FineDataset.collate_fn
        )

        dataset_test = Kitti360FineDatasetMulti(
            args.base_path, SCENE_NAMES_TEST, val_transform, args,)
    ...
    if args.coarse_path_in_fine is not None:
        ...
        model_coarse_dic = torch.load(
            args.coarse_path_in_fine, map_location=torch.device("cpu"))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_out = train_epoch(model, dataloader_train, args, model_coarse)
        ...
        val_out = eval_epoch(model, dataloader_val, args, model_coarse)

        val_stats_pose_offsets[lr].append(val_out.pose_offsets)

        print()

        test_out = eval_epoch(model, dataloader_test, args, model_coarse)
