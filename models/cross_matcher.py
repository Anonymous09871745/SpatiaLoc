...
class CrossMatch(torch.nn.Module):
    def __init__(
        ...
        self.object_encoder = ObjectEncoder(
            args.fine_embed_dim, known_classes, known_colors, args)
    ...
    def forward(self, objects, hints, object_points, object_level_similarity=None, thresh=0.15):
        # fine-localization with Object-level Confidence
        batch_size = len(objects)
        num_objects = len(objects[0])

        # calculate similarity score of object-level
        if object_level_similarity is not None:
            similarity_score = object_level_similarity.transpose(
                1, 2).max(dim=-1)[0]
            row_min = similarity_score.min(
                dim=-1)[0].unsqueeze(-1).repeat(1, num_objects)
            row_max_min = (similarity_score.max(
                dim=-1)[0]-similarity_score.min(dim=-1)[0]).unsqueeze(-1).repeat(1, num_objects)
            similarity_score = (similarity_score-row_min)/(row_max_min)
            similarity_score = similarity_score.transpose(0, 1)
            a = thresh
        """
        Text encoder
        """
        ...
        object_encodings = object_encodings.reshape(
            (batch_size, num_objects, self.embed_dim))
...
def get_pos_in_cell(objects: List[Object3d_K360], matches0, offsets):
    ...
    return (
        np.mean(pose_preds, axis=0) if len(
            pose_preds) > 0 else np.array((0.5, 0.5))
    )  # Guess the middle if no matches


def intersect(P0, P1):
    n = (P1 - P0) / np.linalg.norm(P1 - P0,
                                   axis=1)[:, np.newaxis]  # normalized
...
def get_pos_in_cell_intersect(objects: List[Object3d_K360], matches0, directions):
    ...
    for obj_idx, hint_idx in enumerate(matches0):
        ...
        points1.append(objects[obj_idx].get_center()
                       [0:2] + directions[hint_idx])
    if len(points0) < 2:
